...
import (
	...
	loggingpb "cloud.google.com/go/logging/apiv2/loggingpb" // Ensure this is imported for *loggingpb.LogEntry
...
func (s *CloudLoggingAPISource) Collect(ctx context.Context) error {
	...
	for {
		...
		if logEntry != nil {
			...
			// Convert the higher-level logging.Entry to the Protobuf-generated *loggingpb.LogEntry
			pbEntry, convertErr := logEntry.ToLogEntryProto()
			if convertErr != nil {
				fmt.Printf("Warning: Skipping log entry (InsertID: %s) due to conversion error: %v\n", logEntry.InsertID, convertErr)
				continue
			}
			if pbEntry == nil {
				// If ToLogEntryProto returns nil (e.g., if underlying data is malformed)
				// This is an unlikely scenario for valid logs, but good for robustness.
				fmt.Printf("Warning: Skipped nil *loggingpb.LogEntry after conversion for InsertID %s\n", logEntry.InsertID)
				continue
			}
			...
			if s.CollectionState.ShouldCollect(pbEntry.InsertId, pbEntry.GetTimestamp().AsTime()) {
				row := &types.RowData{
					Data:             pbEntry, // Pass the *loggingpb.LogEntry to the RowData
					SourceEnrichment: sourceEnrichmentFields,
				}

				if err = s.CollectionState.OnCollected(pbEntry.InsertId, pbEntry.GetTimestamp().AsTime()); err != nil {
